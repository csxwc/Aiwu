...
public class BaseUserRecommender {
    ...
    public List<Integer> recommendBasedOnUser(Integer userId) throws TasteException {
        ...
        for (RecommendedItem recommendedItem : recommendedItemList) {
            houseIds.add((int) recommendedItem.getItemID());
        }
...
//        DataModel dataModel = new FileDataModel(file);
        ...
        Recommender recommender = new GenericUserBasedRecommender(dataModel, userNeighborhood, similarity);
        //给用户ID等于5的用户推荐10部电影
        List<RecommendedItem> recommendedItemList = recommender.recommend(1, 6);
        //打印推荐的结果
        System.out.println("使用基于用户的协同过滤算法");
        System.out.println("为用户1推荐6个商品");
        for (RecommendedItem recommendedItem : recommendedItemList) {
